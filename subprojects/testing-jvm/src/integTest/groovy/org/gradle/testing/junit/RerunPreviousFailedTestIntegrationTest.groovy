...
class RerunPreviousFailedTestIntegrationTest extends AbstractIntegrationSpec {
    ...
    @Unroll
    def 'subsequent fail fast execution runs failed test first'() {
        given:
    ...
    void allTestsSucceed() {
        new DefaultTestExecutionResult(testDirectory).assertTestClassesExecuted("ConditionalFailingTest_1", 'ConditionalFailingTest_2', 'ConditionalFailingTest_3')
    }

    void remainTestsSucceed(def indexOfTestToFail) {
        def failedTest = "ConditionalFailingTest_${indexOfTestToFail}".toString()
        def testClasses = ["ConditionalFailingTest_1", 'ConditionalFailingTest_2', 'ConditionalFailingTest_3']
    ...
    void testFailedAndOthersIgnoredOrNotExecuted(def indexOfTestToFail) {
        ...
        testClasses.remove(failedTest)
        result.testClass("ConditionalFailingTest_${indexOfTestToFail}").assertTestFailed('failedTest', Matchers.anything())
        testClasses.each {
